...
exports.submitComplaint = (req, res) => {
  ...
  Complaint.createComplaint(complaint, (err) => {
    if (err) return res.status(500).json({ success: false, message: err.message || 'Error submitting complaint' });
    res.status(201).json({ success: true, message: "Ticket submitted and email sent." });
  });
};

exports.getAllComplaints = (req, res) => {
  Complaint.getAll((err, results) => {
    if (err) return res.status(500).json({ success: false, message: "Failed to fetch complaints" });
...
exports.getUserComplaints = (req, res) => {
  ...
  Complaint.getUserComplaints(email, (err, results) => {
    if (err) return res.status(500).json({ success: false, message: "Error fetching user complaints" });
...
exports.assignPersonnel = (req, res) => {
  ...
  if (!assignedName || !assignedContact) {
    return res.status(400).json({ success: false, message: 'Please provide both name and contact of personnel' });
  }

  Complaint.assignPersonnel(id, assignedName, assignedContact, (err, success) => {
    if (err) return res.status(500).json({ success: false, message: "Failed to assign personnel" });
    if (!success) return res.status(404).json({ success: false, message: "Personnel not found" });
...
exports.resolvedComplaint = (req, res) => {
  ...
  Complaint.markResolved(id, (err) => {
    if (err) return res.status(500).json({ success: false, message: "Failed to resolve complaint" });
...
exports.trackTicket = (req, res) => {
  ...
  Complaint.trackTicket(email, code, (err, results) => {
    ...
    if (results.length === 0) return res.status(404).json({ success: false, message: "Ticket not found" });
